...
export async function createWorkItem(args) {
    ...
    try {
        const workItem = await client.createWorkItem(null, patchDocument, config.project, args.type, false, true);
        return {
            content: [
                {
                    type: 'text',
        ...
        throw new McpError(ErrorCode.InternalError, `Failed to create work item: ${error?.message || 'Unknown error'}`);
    }
}
