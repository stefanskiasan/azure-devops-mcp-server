...
export async function getWikis(args) {
    try {
        ...
        throw new McpError(ErrorCode.InternalError, `Failed to get wikis: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
}
export async function getWikiPage(args) {
    ...
    try {
        const connection = AzureDevOpsConnection.getInstance();
        const wikiApi = new WikiApi(connection);
        const wikiPage = await wikiApi.getWikiPage(args.wikiIdentifier, args.path);
        return {
            content: [
                {
                    type: 'text',
        ...
        throw new McpError(ErrorCode.InternalError, `Failed to get wiki page: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
}
