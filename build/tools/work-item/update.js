...
export async function updateWorkItem(args) {
    ...
    try {
        const workItem = await client.updateWorkItem(null, patchDocument, args.id, config.project, false, true);
        return {
            content: [
                {
                    type: 'text',
        ...
        throw new McpError(ErrorCode.InternalError, `Failed to update work item: ${error?.message || 'Unknown error'}`);
    }
}
